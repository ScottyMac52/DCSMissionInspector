...
Generated by DCS Mission Reader • {DateTime.Now:yyyy-MM-dd HH:mm}
        ...
        {
            var sb = new StringBuilder();
            sb.AppendLine("<h2 id=\"units-targets\">Units &amp; Targets</h2>");

            // Beautiful dark collapsible header
            sb.AppendLine("<details class=\"group mb-8\">");
            sb.AppendLine("  <summary class=\"cursor-pointer flex items-center justify-between px-6 py-5 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-3xl text-xl font-semibold text-slate-100\">");
            sb.AppendLine("    <span class=\"flex items-center gap-3\">");
            sb.AppendLine("      📦 <span>All Ground, Sea &amp; Target Units</span>");
            sb.AppendLine("    </span>");
            sb.AppendLine("    <span class=\"text-2xl transition-transform group-open:rotate-180\">▼</span>");
            sb.AppendLine("  </summary>");

            sb.AppendLine("  <div class=\"mt-6 space-y-8 pl-2\">");

            var coalitionVal = mission.Get("coalition");
            if (coalitionVal.Type != DataType.Table)
            {
                sb.AppendLine("<p class=\"text-emerald-400 italic\">No units or targets found in this mission.</p>");
            ...
            foreach (var side in sides)
            {
                ...
                foreach (var countryPair in countryListVal.Table.Pairs)
                {
                    ...
                    foreach (var category in new[] { "ship", "vehicle", "static" })
                    {
                        ...
                        foreach (var gPair in groupListVal.Table.Pairs)
                        {
                            ...
                            if (unitsVal.Type == DataType.Table)
                            {
                                for (int i = 1; i <= unitsVal.Table.Length; i++)
                                {
                                    var u = unitsVal.Table.Get(i);
                                    if (u.Type != DataType.Table) continue;
                                    string uType = u.Table.Get("type")?.String ?? "Unknown";
                            ...
                            string unitInfo = unitList.Count > 0 ? string.Join(", ", unitList) : "No units";

                            string icon = category == "ship" ? "⚓" : category == "vehicle" ? "🪖" : "📦";
                            string colorClass = side == "blue" ? "blue" : side == "red" ? "red" : "slate";

                            sideHtml.AppendLine($@"
                        <div class=""flex items-start gap-6 p-6 bg-slate-800 border border-slate-700 rounded-3xl hover:border-{colorClass}-500 transition-all group/item"">
                            ...
                            <div class=""flex-1"">
                                ...
                                <div class=""text-sm text-slate-300 mt-3 line-clamp-3"">{unitInfo}</div>
                            </div>
                ...
                if (hasUnits)
                {
                    string sideColor = side == "blue" ? "blue" : side == "red" ? "red" : "slate";
                    sb.AppendLine($"<h3 class=\"text-{sideColor}-400 text-lg font-semibold mt-10 mb-5 flex items-center gap-2\">");
                    sb.AppendLine($"  <span class=\"text-2xl\">{(side == "blue" ? "🔵" : side == "red" ? "🔴" : "⚪")}</span>");
                    sb.AppendLine($"  {side.ToUpper()} COALITION");
                    sb.AppendLine("</h3>");
                    sb.Append(sideHtml);
                }
            }

            if (!sb.ToString().Contains("COALITION"))
                sb.AppendLine("<p class=\"text-emerald-400 italic\">No ground, sea, or target units found in this mission.</p>");
        ...
        */
        ...
        // UPDATED: GenerateKmlExport - UNIQUE COLORS per route (sensible coalition colors)
        private static void GenerateKmlExport(string reportDir, string sortie, Table mission)
        {
            ...
            // Waypoint placemark style (unchanged)
                    ...
                    foreach (var countryPair in countryListVal.Table.Pairs)
                    {
                        ...
                        foreach (var cat in new[] { "plane", "helicopter" })
                        {
                            ...
                            foreach (var groupPair in groupListVal.Table.Pairs)
                            {
                                ...
                                // Pick color based on coalition
                                string color;
                                if (side == "blue")
                                    color = blueColors[blueIdx++ % blueColors.Length];
                                else if (side == "red")
                                    color = redColors[redIdx++ % redColors.Length];
                                else
                                    color = neutralColors[neutralIdx++ % neutralColors.Length];
                                ...
                                // === WAYPOINT PLACEMARKS (numbered/named) ===
                                for (int i = 1; i <= pointsVal.Table.Length; i++)
                                {
                                    var pt = pointsVal.Table.Get(i);
                                    if (pt.Type != DataType.Table) continue;
            ...
            Console.WriteLine($"   🗺️  KML created with unique colored routes + waypoint placemarks → {kmlPath}");
        }
